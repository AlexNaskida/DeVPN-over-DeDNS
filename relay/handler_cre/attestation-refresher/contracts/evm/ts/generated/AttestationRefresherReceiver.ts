...
/**
 * Filter params for AttestationRefreshed. Only indexed fields can be used for filtering.
 * Indexed string/bytes must be passed as keccak256 hash (Hex).
 */
export type AttestationRefreshedTopics = {
  node?: `0x${string}`
}

/**
 * Decoded AttestationRefreshed event data.
 */
export type AttestationRefreshedDecoded = {
  node: `0x${string}`
  newHash: `0x${string}`
}
...
export class AttestationRefresherReceiver {
  ...
  rESOLVER(
    ...
    const callData = encodeFunctionData({
      abi: AttestationRefresherReceiverABI,
  ...
  /**
   * Creates a log trigger for AttestationRefreshed events.
   * The returned trigger's adapt method decodes the raw log into AttestationRefreshedDecoded,
   ...
   */
  logTriggerAttestationRefreshed(
    filters?: AttestationRefreshedTopics[],
  ) {
    let topics: { values: string[] }[]
    if (!filters || filters.length === 0) {
      const encoded = encodeEventTopics({
        abi: AttestationRefresherReceiverABI,
        eventName: 'AttestationRefreshed' as const,
      })
    ...
    return {
      ...
      adapt: (rawOutput: EVMLog): DecodedLog<AttestationRefreshedDecoded> => contract.decodeAttestationRefreshed(rawOutput),
    }
  }

  /**
   * Decodes a log into AttestationRefreshed data, preserving all log metadata.
   */
  decodeAttestationRefreshed(log: EVMLog): DecodedLog<AttestationRefreshedDecoded> {
    ...
    return { ...rest, data: decoded.args as unknown as AttestationRefreshedDecoded }
  }
